(function () {
  ...
  function buildTextReceipt(data) {
    ...
    if (data.subtitle) {
      lines.push(center(String(data.subtitle).slice(0, width), width));
    }
    lines.push(divider);
    ...
    if (data.footer) {
      String(data.footer)
        .split(/\r?\n/)
        .forEach(function (f) {
          lines.push(center(f.slice(0, width), width));
        });
    }
    lines.push("\n\n"); // feed a bit
    return lines.join("\n");
  ...
  function buildHtmlReceipt(data) {
    var itemsHtml = (data.Items || [])
      .map(function (it) {
        ...
        return (
          ...
          "</div>" +
          '<div class="amt">' +
          escapeHtml(peso(subtotal)) +
          "</div>" +
          '<div class="meta">' +
          qty +
          " × " +
          escapeHtml(peso(unit)) +
          "</div>" +
          "</div>"
        );
      })
    ...
    var subtitle = data.subtitle
      ? '<div class="subtitle">' + escapeHtml(String(data.subtitle)) + "</div>"
      : "";
    var ref = data.ReferenceNo
      ? '<div class="line">Ref: ' + escapeHtml(data.ReferenceNo) + "</div>"
      : "";
    var pay = data.PaymentMethod
      ? '<div class="line">Pay: ' + escapeHtml(data.PaymentMethod) + "</div>"
      : "";
    var pickup = data.PickupAt
      ? '<div class="line">Pickup: ' + escapeHtml(data.PickupAt) + "</div>"
      : "";
    var css = [
    ...
    return (
      ...
      escapeHtml(String(data.OrderID)) +
      "</div>" +
      ref +
      '<div class="line">Date: ' +
      escapeHtml(String(data.OrderDate)) +
      "</div>" +
      pay +
      pickup +
      '<div class="div"></div>' +
      itemsHtml +
      '<div class="div"></div>' +
      '<div class="total">TOTAL <span style="float:right">' +
      escapeHtml(peso(data.TotalAmount)) +
      "</span></div>" +
      '<div class="div"></div>' +
  ...
  async function printViaQZ(html) {
    try {
      ...
      if (!qz.websocket.isActive()) {
        await qz.websocket.connect().catch(function () {
          /* ignore */
        });
      }
      if (!qz.websocket.isActive()) return false;
      var printer = await qz.printers.getDefault().catch(function () {
        return null;
      });
    ...
    try {
      ...
      if (window.LA_USE_LAN_PRINTER === true) {
        try {
          ...
          if (resLan.ok) {
            var j = await resLan.json().catch(function () {
              return {};
            });
