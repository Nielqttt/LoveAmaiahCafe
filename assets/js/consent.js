(function(){
  ...
  function writeConsent(obj){
    ...
    let cookie = COOKIE_NAME + '=' + val + '; max-age=' + oneYear + '; path=/; SameSite=Lax';
    if (location.protocol === 'https:') cookie += '; Secure';
    document.cookie = cookie;
    document.dispatchEvent(new CustomEvent('la:consent-updated', { detail: obj }));
  }
    ...
    return new Promise((resolve,reject)=>{
      ...
      if (attrs) {
        Object.keys(attrs).forEach(k=>{
          ...
          else if (attrs[k] !== false && attrs[k] != null) s.setAttribute(k, attrs[k]);
        });
      }
...
`;
    ...
    div.querySelector('#la-accept').addEventListener('click', ()=>{
      writeConsent({necessary:true,functional:true,analytics:true,marketing:true,ts:new Date().toISOString(),v:1});
      div.remove();
    });
    div.querySelector('#la-reject').addEventListener('click', ()=>{
      writeConsent({necessary:true,functional:false,analytics:false,marketing:false,ts:new Date().toISOString(),v:1});
      div.remove();
    });
  }
  ...
  const api = {
    ...
    set(consent){ writeConsent(ensureDefaults(consent)); },
    allow(type){ const c = readConsent(); return !!(c && c[type]); },
    load(type, src, attrs){ if (this.allow(type)) { return loadScript(src, attrs); } return Promise.resolve(false); },
    ensureBanner(){ if (!readConsent()) injectBanner(); }
  };
