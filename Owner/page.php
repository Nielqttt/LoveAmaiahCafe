...
<html lang="en">
 ...
 <body class="bg-[rgba(255,255,255,0.7)] min-h-screen md:h-screen flex flex-col md:flex-row md:overflow-hidden"><!-- Added md:h-screen so inner menu section can scroll independently -->
  ...
  <div class="md:hidden flex items-center justify-between px-4 py-2 bg-white/90 backdrop-blur-sm shadow sticky top-0 z-30">
    ...
    <div class="flex items-center gap-2">
      <button id="mobile-order-toggle" class="relative px-3 py-1 rounded-full bg-[#4B2E0E] text-white text-sm font-semibold">
        Order (<span id="order-count">0</span>)
      </button>
      <button id="mobile-nav-toggle" class="p-2 rounded-full border border-[#4B2E0E] text-[#4B2E0E]">
...
?>;
  ...
  img.className = "mb-2 w-20 h-20 object-cover rounded";
   ...
   function renderOrder() {
     // Clear list but keep external heading intact
     orderList.innerHTML = '';
    ...
    Swal.fire({
      ...
      html: `
        <div style="text-align:left">
          <div style="border-radius:28px;padding:16px;background:#ffffff;box-shadow:0 8px 24px rgba(0,0,0,0.08)">
            <img src="${imgSrc}" alt="${name}" style="width:100%;height:auto;border-radius:22px;object-fit:cover;display:block;margin-bottom:12px" />
