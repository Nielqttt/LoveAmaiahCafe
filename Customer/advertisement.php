...
<head>
  ...
  <style>
    :root { --main-color:#a17850; --white:#fff; --spacing:2rem; --card-bg:#2d2d2d; --accent:#4B2E0E; --light-brown:#C4A07A; }
    *{ box-sizing:border-box; margin:0; padding:0; }
    body{ font-family:'Segoe UI', sans-serif; background:url('../images/LAbg.png') no-repeat center/cover; min-height:100vh; color:var(--white); overflow-x:hidden; scroll-behavior:smooth; }
    ...
    .hero-text h1{ font-size:4.5rem; line-height:1.2; margin-bottom:1.5rem; letter-spacing:-1px; font-weight:800; }
    ...
    @media (max-width:1024px){ .hero-text h1{ font-size:3.2rem; } .hero-text p{ font-size:1.4rem; } }
    @media (max-width:768px){ .hero{ flex-direction:column; gap:3rem; padding:1rem 0; } .hero-text{ padding:1rem; text-align:center; } .hero-text h1{ font-size:2.8rem; } .hero-text p{ font-size:1.2rem; } .main-content{ padding:1rem; } }
    @media (max-width:480px){ .main-content{ padding:1.25rem; } .coffee-cards{ grid-template-columns:1fr; } .hero-text h1{ font-size:2rem; } .hero-text p{ font-size:1.05rem; } .section-sub{ font-size:1.05rem; } .hero-text button{ width:100%; } }
  </style>
</head>
<body class="bg-cover bg-center bg-no-repeat" style="background-image:url('../images/LAbg.png')">
  ...
  <div class="md:hidden flex items-center justify-between px-4 py-2 bg-white/90 backdrop-blur-sm shadow sticky top-0 z-30">
    ...
    <div class="flex items-center gap-2">
      <button id="mobile-nav-toggle" class="p-2 rounded-full border border-[#4B2E0E] text-[#4B2E0E]" aria-label="Open navigation"><i class="fa-solid fa-bars"></i></button>
    </div>
  </div>
  ...
  <div class="page-shell">
    ...
    <main class="main-content">
        ...
        <div class="hero-overlay">
          <div class="hero-text">
            ...
            <div class="hero-cta">
              ...
              <button onclick="location.href='../all/login.php'" aria-label="Order now">Order Now</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="coffee-cards fade-in" id="menu">
        ...
        <div class="card"><img src="../images/affogato.png" alt="Affogato coffee dessert" loading="lazy"><div class="card-body"><h3>Affogato</h3><p>Espresso poured over vanilla ice cream — bold, creamy, and decadent.</p></div></div>
        <div class="card"><img src="../images/caramel_cloud_latte.png" alt="Caramel Cloud Latte" loading="lazy"><div class="card-body"><h3>Caramel Cloud Latte</h3><p>Fluffy foam, bold espresso, and silky caramel — heavenly in every sip.</p></div></div>
        <div class="card"><img src="../images/cinnamon_macchiato.png" alt="Cinnamon Macchiato" loading="lazy"><div class="card-body"><h3>Cinnamon Macchiato</h3><p>Warm cinnamon meets espresso and milk — sweet, spicy, and smooth.</p></div></div>
        <div class="card"><img src="../images/iced_shaken_brownie.png" alt="Iced Brownie Espresso" loading="lazy"><div class="card-body"><h3>Iced Brownie Espresso</h3><p>Shaken espresso with rich brownie flavor — bold, cold, and energizing.</p></div></div>
      </section>
      ...
      <section id="visit" class="fade-in" style="display:grid; gap:.6rem;">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:.5rem;align-items:stretch;">
          <div style="width:100%;"><div style="border-radius:20px;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,.4);height:100%;">
  ...
  <script>
    ...
    (function(){
      const slides = document.querySelectorAll('.slides img'); if(!slides.length) return;
      let i = 0; setInterval(()=>{ slides[i].classList.remove('active'); i = (i + 1) % slides.length; slides[i].classList.add('active'); }, 4000);
    })();

    // Auto-scroll gallery
    (function(){
      const scroller = document.querySelector('.scroll-gallery'); if(!scroller) return;
      let isPaused = false; const prefersReduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches; const speed = prefersReduced ? 40 : 160;
      const original = Array.from(scroller.children); const cloneOnce = () => original.forEach(n=> scroller.appendChild(n.cloneNode(true))); cloneOnce(); if(scroller.scrollWidth <= scroller.clientWidth + 32) cloneOnce();
      let singleWidth = scroller.scrollWidth / 2; let startX=0, startScroll=0, dragging=false; const pageX=e=> (e.touches?e.touches[0].pageX:e.pageX);
      const pause=()=>{isPaused=true}, resume=()=>{isPaused=false};
      ...
      scroller.addEventListener('mousedown', onDown); scroller.addEventListener('touchstart', onDown, {passive:true});
      ...
      let last = performance.now(); const loop=now=>{ const dt=(now-last)/1000; last=now; if(!isPaused){ scroller.scrollLeft += speed*dt; if(scroller.scrollLeft >= singleWidth){ scroller.scrollLeft -= singleWidth; } } requestAnimationFrame(loop); }; requestAnimationFrame(t=>{ last=t; loop(t); });
    })();
    ...
    (function(){
      ...
      mobileNavToggle?.addEventListener('click', open); mobileNavClose?.addEventListener('click', close); mobileNavBackdrop?.addEventListener('click', close);
    })();

    // Logout
    (function(){
      const logoutBtn = document.getElementById('logout-btn'); const logoutBtnMobile = document.getElementById('logout-btn-mobile');
      function handleLogout(){
        if(window.Swal){ Swal.fire({ title:'Are you sure you want to log out?', icon:'warning', showCancelButton:true, confirmButtonColor:'#4B2E0E', cancelButtonColor:'#d33', confirmButtonText:'Yes, log out', cancelButtonText:'Cancel' }).then(r=>{ if(r.isConfirmed){ window.location.href = '../all/logoutcos.php'; }}); }
        else { if(confirm('Log out?')) window.location.href = '../all/logoutcos.php'; }
      }
      logoutBtn?.addEventListener('click', e=>{ e.preventDefault(); handleLogout(); }); logoutBtnMobile?.addEventListener('click', e=>{ e.preventDefault(); handleLogout(); });
    })();
  </script>
</body>
</html>
